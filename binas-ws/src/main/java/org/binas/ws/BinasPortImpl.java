	...
	public UserView activateUser(String email) throws InvalidEmail_Exception, EmailExists_Exception{
		try {
			...
			try {
				...
				System.err.println("Caught exception while activating user: " + e);
			}
			...
			for(String stationUrl : stationUrls) {
					...
					try {
						client = new StationClient(stationUrl);
						try {
							if(client.getBalance(email) == null) {
								client.setBalance(email,10,0);
								BalanceView x = client.getBalance(email);
								System.out.println("Added: "+ email+ " with "+ x.getCredit()+ " credits TAG: " + x.getTag());
							}
							else {
...
//		return 0;
		...
		try {
			view = BinasManager.getInstance().quorumRead(email);
		} catch (StationNotFoundException | NoUserFound_Exception e) {
			System.err.println("Caught exception on getCredit: " + e);
		}
