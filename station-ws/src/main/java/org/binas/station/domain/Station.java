...
public class Station {
	...
	/*NEW HASMAP TO KEEP USERS (email,BalanceView)*/
	private Map<String, BalanceView> UsersList = new ConcurrentHashMap<>();
	...
	public synchronized BalanceView getBalance(String email) {
		return UsersList.get(email);	
    }
	
	public synchronized void setBalance(String email, BalanceView view) throws InvalidCreditException {
		if (view.getCredit() < 0)
			throw new InvalidCreditException();
		if (UsersList.containsKey(email) == false) {
			UsersList.put(email, view);
		}
		else UsersList.replace(email, view);
    }
